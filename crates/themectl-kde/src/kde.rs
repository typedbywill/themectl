...
use themectl_spec::ThemeManifest;
use themectl_utils::cmd::{check_tool, run};
use themectl_utils::fs::{copy_dir, ensure_dir};
use std::path::Path;
...
pub fn apply(theme_dir: &Path, manifest: &ThemeManifest, opts: ApplyOptions) -> Result<ApplyReport> {
    ...
    let local_data = dirs::data_local_dir().ok_or_else(|| {
        themectl_utils::ThemectlError::Io(std::io::Error::new(
